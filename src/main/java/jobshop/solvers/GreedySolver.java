...
public class GreedySolver implements Solver {
    ...
    //It is the constructor of the class, it determines if the choice of tasks is made with SPT or LRPT
    public GreedySolver(EnumPriority enumPriority) {
    ...
    private Task best_task(ArrayList<Task> tasks) {
        ...
        //SPT -- priority to the shortest task
        if (this.enumPriority == EnumPriority.SPT) {
            ...
            for (int i = 1; i < tasks.size(); i++) {
                Task current_task = tasks.get(i);
                //added one more condition than SPT, we look at the release date of the maĉhines
                if (this.machines_release[this.instance.machine(taskN1.job, taskN1.task)] > this.machines_release[this.instance.machine(current_task.job, current_task.task)]) {
                    //comparison of the duration with the optimum (taskN1)
                    if (this.instance.duration(taskN1.job, taskN1.task) > this.instance.duration(current_task.job, current_task.task)) {
                        taskN1 = current_task;
                    }
    ...
    public Result solve(Instance instance, long deadline) {
        ...
        //management of remaining time if SRPT or LRPT
        if (this.enumPriority == EnumPriority.LRPT || this.enumPriority == EnumPriority.EST_LRPT ) {
